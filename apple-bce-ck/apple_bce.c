...
static int bce_save_state_and_sleep(struct apple_bce_device *bce)
{
    ...
    pr_info("apple-bce_ck: SAVE_STATE_AND_SLEEP enter, initial size=%zu\n", size);
    for (attempt = 0; attempt < 5; ++attempt) {
        pr_info("apple-bce_ck: SAVE_STATE attempt %d/5, size=%zu\n", attempt + 1, size);
        dma_ptr = dma_alloc_coherent(&bce->pci->dev, size, &dma_addr, GFP_KERNEL);
        if (!dma_ptr) {
            pr_err("apple-bce_ck: SAVE_STATE dma_alloc_coherent FAILED at attempt %d\n", attempt + 1);
            break;
        }
        pr_info("apple-bce_ck: SAVE_STATE dma_alloc OK attempt %d, dma_addr=0x%llx\n", attempt + 1, dma_addr);
        BUG_ON((dma_addr % 4096) != 0);
        pr_info("apple-bce_ck: SAVE_STATE sending mailbox cmd attempt %d\n", attempt + 1);
        status = bce_mailbox_send(&bce->mbox,
                BCE_MB_MSG(BCE_MB_SAVE_STATE_AND_SLEEP, (dma_addr & ~(4096LLU - 1)) | (size / 4096)), &resp);
        pr_info("apple-bce_ck: SAVE_STATE mailbox returned status=%d attempt %d\n", status, attempt + 1);
        if (status) {
            pr_err("apple-bce_ck: SAVE_STATE mailbox send FAILED status=%d at attempt %d\n", status, attempt + 1);
            break;
        }
        pr_info("apple-bce_ck: SAVE_STATE response type=0x%llx attempt %d\n", BCE_MB_TYPE(resp), attempt + 1);
        if (BCE_MB_TYPE(resp) == BCE_MB_SAVE_RESTORE_STATE_COMPLETE) {
            pr_info("apple-bce_ck: SAVE_STATE COMPLETE at attempt %d, size=%zu\n", attempt + 1, size);
            ...
            pr_info("apple-bce_ck: SAVE_STATE device asked for larger buffer %zu, retrying\n", size);
            continue;
        } else {
            pr_err("apple-bce_ck: SAVE_STATE invalid response type 0x%llx\n", BCE_MB_TYPE(resp));
            status = -EINVAL;
            break;
        }
    }
    pr_info("apple-bce_ck: SAVE_STATE loop exhausted, status=%d, attempts=%d\n", status, attempt);
    if (dma_ptr) {
        pr_info("apple-bce_ck: SAVE_STATE freeing DMA buffer\n");
        dma_free_coherent(&bce->pci->dev, size, dma_ptr, dma_addr);
    }
    if (!status) {
        pr_info("apple-bce_ck: SAVE_STATE sending SLEEP_NO_STATE as fallback\n");
        return bce_mailbox_send(&bce->mbox, BCE_MB_MSG(BCE_MB_SLEEP_NO_STATE, 0), &resp);
    }
    pr_info("apple-bce_ck: SAVE_STATE returning error status=%d\n", status);
    return status;
}

static int apple_bce_restore_state_and_wake(struct apple_bce_device *bce)
{
    int status;
    u64 resp;
    if (!bce->saved_data_dma_ptr) {
        pr_info("apple-bce_ck: RESTORE no saved state, sending RESTORE_NO_STATE\n");
        if ((status = bce_mailbox_send(&bce->mbox, BCE_MB_MSG(BCE_MB_RESTORE_NO_STATE, 0), &resp))) {
            pr_err("apple-bce_ck: RESTORE_NO_STATE mailbox FAILED status=%d\n", status);
            return status;
        }
        if (BCE_MB_TYPE(resp) != BCE_MB_RESTORE_NO_STATE) {
            pr_err("apple-bce_ck: RESTORE_NO_STATE invalid response type=0x%llx\n", BCE_MB_TYPE(resp));
            return -EINVAL;
        }
        pr_info("apple-bce_ck: RESTORE_NO_STATE OK\n");
        return 0;
    }

    pr_info("apple-bce_ck: RESTORE has saved state, sending RESTORE_STATE_AND_WAKE\n");
    if ((status = bce_mailbox_send(&bce->mbox, BCE_MB_MSG(BCE_MB_RESTORE_STATE_AND_WAKE,
        ...
        pr_err("apple-bce_ck: RESTORE_STATE_AND_WAKE mailbox FAILED status=%d\n", status);
        goto finish_with_state;
    }
    if (BCE_MB_TYPE(resp) != BCE_MB_SAVE_RESTORE_STATE_COMPLETE) {
        pr_err("apple-bce_ck: RESTORE_STATE_AND_WAKE invalid response type=0x%llx\n", BCE_MB_TYPE(resp));
        status = -EINVAL;
        goto finish_with_state;
    }
    pr_info("apple-bce_ck: RESTORE_STATE_AND_WAKE OK\n");

finish_with_state:
    pr_info("apple-bce_ck: RESTORE freeing saved DMA buffer\n");
...
static int apple_bce_suspend(struct device *dev)
{
    ...
    pr_info("apple-bce_ck: SUSPEND enter\n");
    bce_timestamp_stop(&bce->timestamp);
    pr_info("apple-bce_ck: SUSPEND timestamp stopped, calling save_state_and_sleep\n");

    if ((status = bce_save_state_and_sleep(bce))) {
        pr_err("apple-bce_ck: SUSPEND FAILED status=%d\n", status);
        return status;
    }

    pr_info("apple-bce_ck: SUSPEND done, returning 0\n");
    return 0;
}
...
static int apple_bce_resume(struct device *dev)
{
    ...
    pr_info("apple-bce_ck: RESUME enter\n");
    pci_set_master(bce->pci);
    pci_set_master(bce->pci0);
    pr_info("apple-bce_ck: RESUME PCI masters set, calling restore\n");

    if ((status = apple_bce_restore_state_and_wake(bce))) {
        pr_err("apple-bce_ck: RESUME FAILED status=%d\n", status);
        return status;
    }

    pr_info("apple-bce_ck: RESUME starting timestamp\n");
    bce_timestamp_start(&bce->timestamp, false);
    pr_info("apple-bce_ck: RESUME done, returning 0\n");
    return 0;
}
...
static int apple_bce_prepare(struct device *dev) { pr_info("apple-bce_ck: PREPARE called\n"); return 0; }
static void apple_bce_complete(struct device *dev) { pr_info("apple-bce_ck: COMPLETE called\n"); pm_runtime_get(dev); }
