...
static int aaudio_suspend(struct device *dev)
{
    ...
    pr_info("apple-bce_ck: AAUDIO_SUSPEND enter\n");
    ret = aaudio_cmd_set_remote_access(aaudio, AAUDIO_REMOTE_ACCESS_OFF);
    if (ret) {
        dev_warn(aaudio->dev, "aaudio_suspend: set_remote_access(OFF) returned %d\n", ret);
    } else {
        pr_info("apple-bce_ck: AAUDIO_SUSPEND remote_access OFF OK\n");
    }

    pci_disable_device(aaudio->pci);
    pr_info("apple-bce_ck: AAUDIO_SUSPEND PCI disabled, returning 0\n");
    return 0;
}
...
static int aaudio_resume(struct device *dev)
{
    ...
    pr_info("apple-bce_ck: AAUDIO_RESUME enter\n");
    if ((status = pci_enable_device(aaudio->pci))) {
        dev_err(aaudio->dev, "aaudio_resume: pci_enable_device failed %d\n", status);
        return status;
    }
    pci_set_master(aaudio->pci);
    pr_info("apple-bce_ck: AAUDIO_RESUME PCI enabled, setting remote_access ON\n");

    if ((status = aaudio_cmd_set_remote_access(aaudio, AAUDIO_REMOTE_ACCESS_ON))) {
        dev_err(aaudio->dev, "aaudio_resume: set_remote_access(ON) failed %d\n", status);
        return status;
    }
    pr_info("apple-bce_ck: AAUDIO_RESUME remote_access ON OK, done\n");
...
{
    struct aaudio_timestamp_work_struct *work = container_of(ws, struct aaudio_timestamp_work_struct, ws);
    struct aaudio_subdevice *sdev;

    sdev = aaudio_find_dev_by_dev_id(work->a, work->devid);
    if (sdev)
        aaudio_handle_timestamp(sdev, work->time_os, work->timestamp);
    else
        dev_err(work->a->dev, "Timestamp update: device not found\n");
    kfree(work);
}
...
void aaudio_handle_cmd_timestamp(struct aaudio_device *a, struct aaudio_msg *msg)
{
    ...
    /* aaudio_handle_timestamp() calls snd_pcm_period_elapsed(), which for a
     * non-atomic PCM may run the .trigger(STOP) callback and issue a *synchronous*
     * BCE command (aaudio_cmd_stop_io -> wait_for_completion_timeout). This handler
     * runs from the BCE DMA completion path with bce->queues_lock held (see
     * bce_handle_dma_irq()), i.e. in atomic context, so doing it inline triggers
     * "scheduling while atomic" -- and, with the un-maskable iTCO watchdog on T2,
     * a reboot -- on resume. Defer to process context, mirroring
     * aaudio_handle_prop_change(). */
    work = kmalloc(sizeof(*work), GFP_ATOMIC);
    if (work) {
        work->a = a;
        work->devid = devid;
        work->time_os = time_os;
        work->timestamp = timestamp;
        INIT_WORK(&work->ws, aaudio_handle_timestamp_work);
        schedule_work(&work->ws);
    } else {
        dev_err(a->dev, "Timestamp update: failed to allocate work\n");
    }
...
static int aaudio_prepare(struct device *dev) { pr_info("apple-bce_ck: AAUDIO_PREPARE called\n"); return 0; }
static void aaudio_complete(struct device *dev) { pr_info("apple-bce_ck: AAUDIO_COMPLETE called\n"); pm_runtime_get(dev); }

struct dev_pm_ops aaudio_pci_driver_pm = {
        .prepare = aaudio_prepare,
        .complete = aaudio_complete,
